	...
	{	
		...
		{		
			...
			if( err == errNone )
			{
				...
				{
					// Filter for .wav
					if( 	StrCompare(fileName + StrLen(fileName) - 4, ".wav") == 0
						||	fileInfo.attributes == vfsFileAttrDirectory )
					{
						PrvAddRecord(&fileInfo);
					}
...
static void PrvListViewLoadTable(FormType *frmP, Boolean redraw)
{
	...
	//If scrolling has been used and there is only 1 file, reset gTopIndex

	if (gScrolling == true && gFileCount == 1){
		gTopIndex = 0;
