...
#define	accept(socket,addr,addrlen)							\
				NetLibSocketAccept(socket,(NetSocketAddrType*)addr,(Int16 *)addrlen,AppNetTimeout,&errno)
				
#define	bind(socket,localaddr,addrlen)						\
				NetLibSocketBind(socket,(NetSocketAddrType*)localaddr,addrlen,AppNetTimeout,&errno)
				
#define	close(socket) 												\
				NetLibSocketClose(socket,AppNetTimeout,&errno)
				
#define	connect(socket,addr,addrlen)							\
				NetLibSocketConnect(socket,(NetSocketAddrType*)addr,addrlen,AppNetTimeout,&errno)
				
#define	getsockopt(socket,level,opt,optvalP,optlenP)		\
				NetLibSocketOptionGet(socket,level,opt,optvalP,(UInt16 *)optlenP,AppNetTimeout,&errno)
				
#define	listen(socket,queuelen)									\
				NetLibSocketListen(socket,queuelen,AppNetTimeout,&errno)
				
#define	read(socket,buffer,buflen)								\
				NetLibReceive(socket,buffer,buflen,0,0,0,AppNetTimeout,&errno)
				
#define	recv(socket,buffer,buflen,flags)						\
				NetLibReceive(socket,buffer,buflen,flags,0,0,AppNetTimeout,&errno)
				
#define	recvfrom(socket,buffer,buflen,flags,from,fromlen) \
				NetLibReceive(socket,buffer,buflen,flags,from,(UInt16 *)fromlen,AppNetTimeout,&errno)
				
#define	recvmsg(socket,msg,flags)								\
				NetLibReceivePB(socket,(NetIOParamType*)msg,flags,AppNetTimeout,&errno)
				
#define	send(socket,buf,buflen,flags)							\
				NetLibSend(socket,buf,buflen,flags,0,0,AppNetTimeout,&errno)
				
#define	sendmsg(socket,msg,flags)								\
				NetLibSendPB(socket,(NetIOParamType*)msg,flags,AppNetTimeout,&errno)
				
#define	sendto(socket,buf,buflen,flags,to,tolen)			\
				NetLibSend(socket,buf,buflen,flags,to,tolen,AppNetTimeout,&errno)
				
#define	setsockopt(socket,level,opt,optvalP,optlen)		\
				NetLibSocketOptionSet(socket,level,opt,optvalP,optlen,AppNetTimeout,&errno)
				
#define	shutdown(socket,direction)								\
				NetLibSocketShutdown(socket,direction,AppNetTimeout,&errno)
				
#define	socket(domain,type,protocol)							\
				NetLibSocketOpen(domain,type,protocol,AppNetTimeout,&errno)

#define	write(socket,buf,buflen)								\
				NetLibSend(socket,(void *) buf,buflen,0,0,0,AppNetTimeout,&errno)
				
#define	getpeername(socket,nameP,nameLenP)					\
				NetLibSocketAddr(socket, 0, 0, (NetSocketAddrType*)nameP, (Int16 *)nameLenP, AppNetTimeout, &errno)			
				
#define	getsockname(socket,nameP,nameLenP)					\
				NetLibSocketAddr(socket, (NetSocketAddrType*)nameP, (Int16 *)nameLenP, 0, 0, AppNetTimeout, &errno)			
