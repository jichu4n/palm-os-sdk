...
static Err PrvFormInit(FormType *frmP)
{
	...
	*gCurrentFile = NULL;
	...
	//
	{
		...
		Err err = errNone;
		DmOpenRef dbP = DmOpenDatabaseByTypeCreator('appl', 'mBuz', dmModeReadOnly); // Make sure we use our own bitmaps...
	...
	{	
		...
		{		
			...
			if( err == errNone )
			{
				...
				{
					// Filter for .wav
					if( 	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".wav") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".amr") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".qcp") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".mp3") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 5, ".apcm") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".m4a") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".mp4") == 0
						||	StrCaselessCompare(fileName + StrLen(fileName) - 4, ".aac") == 0
						||	fileInfo.attributes == vfsFileAttrDirectory )
					{
						PrvAddRecord(&fileInfo);
					}
