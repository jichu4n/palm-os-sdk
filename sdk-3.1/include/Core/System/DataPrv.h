...
typedef struct DmAccessType {
	...
	// NEW FOR PalmOS v3.0
	DWord						savedModNum;			// saved modification number of database
...
typedef struct {
		Word		indexes[MAX_STORES];
		UInt		cardNos[MAX_STORES];						// cardNo for each store
		DWord		latestVersionCandidateType;			// saved type of latest version candidate
		DWord		latestVersionCandidateCreator;		// saved creator of latest version candidate
		Int		latestVersionCandidateStore;			// saved store for latest versions
