...
typedef struct DlpVersionType {
												// OFFSET
	Word					wMajor;			// 0;		major version number (0 = ignore)
	Word					wMinor;			// 2;		minor version number
												// TOTAL: 4 bytes										
	} DlpVersionType;
...
typedef struct DlpReadSysInfoVerRespType {
	...
	DWord					dwMaxRecSize;	// 8;		maximum record/resource size that may be allocated on
...
typedef struct DlpCardInfoHdrType {
	...
	DWord					freeRam;			// 20;	total free data store RAM - Fixed in DLP v1.2 to exclude
												//			dynamic heap RAM
...
typedef struct DlpDBInfoHdrType {
												// OFFSET
	Byte					totalSize;		// 0;		total size of the DB info (DlpDBInfoHdrType + name)
												//			*ROUNDED UP TO EVEN SIZE*
	Byte					miscFlags;		//	1;		dlpDbInfoMiscFlag... flags(v1.1) -- set all unused bits to null!
	...
	Word					dbIndex;			//	42;	DB index (or dlpDbInfoUnknownDbIndex for dlpFindDB)
												// TOTAL: 44 bytes;
	} DlpDBInfoHdrType;
...
// DB responses.
typedef Byte DlpCloseDBReqType;
...
typedef struct DlpCloseDBExReqType {
												// OFFSET
	Byte					dbID;				//	0;		database id to close
	Byte					bOptFlags;		// 1;		bitfield of dlpCloseDBExOptFlag... flags
												// TOTAL: 2 bytes										
	} DlpCloseDBExReqType;
...
typedef struct DlpFindDBByNameReqHdrType {
												// OFFSET
	Byte					bOptFlags;		// 0;		bitfield of dlpFindDBOptFlag... flags
	Byte					bCardNo;			// 2;		card number to search
												// TOTAL: 4 bytes										
...
typedef struct DlpFindDBByNameReqType {
	...
	DlpFindDBByNameReqHdrType			//
							header;			// 0;		fixed size header
												// FIXED SIZE: 4 bytes
	Byte					name[1];			// variable size -- zero-terminated database name string
...
typedef struct DlpFindDBByOpenHandleReqType {
												// OFFSET
	Byte					bOptFlags;		// 0;		bitfield of dlpFindDBOptFlag... flags
	Byte					bDbID;			// 1;		database id returned by dlpOpenDB or dlpCreateDB
												// TOTAL: 2 bytes										
...
typedef struct DlpFindDBByTypeCreatorReqType {
												// OFFSET
	Byte					bOptFlags;		// 0;		bitfield of dlpFindDBOptFlag... flags
	Byte					bSrchFlags;		// 1;		bitfield of dlpFindDBSrchFlag... flags
	DWord					dwType;			// 2;		db type id (zero = wildcard)
	DWord					dwCreator;		// 6;		db creator id (zero = wildcard)
												// TOTAL: 10 bytes
...
typedef struct DlpFindDBBasicRespHdrType {
												// OFFSET
	Byte					bCardNo;			// 0;		card number of database
	Byte					bReserved;		// 1;		RESERVED -- SET TO NULL
	DWord					dwLocalID;		// 2;		local id of the database (for internal use)
	DWord					dwOpenRef;		// 6;		db open ref of the database if it is currently opened
...
typedef struct DlpFindDBSizeRespType {
	...
	DWord					dwTotalBytes;	// 4;		total bytes used by db
	DWord					dwDataBytes;	// 8;		bytes used for data
	
	// Returned if dlpFindDBOptFlagGetSize is set for dlpFindDBByOpenHandleReqArgID only:
	// (otherwise, fields are set to zero)
	DWord					dwAppBlkSize;	//12;		size of app info block size (for
												//			dlpFindDBByOpenHandleReqArgID only)
	DWord					dwSortBlkSize;	//16;		size of sort info block size(for
												//			dlpFindDBByOpenHandleReqArgID only)
												//
	// Returned if dlpFindDBOptFlagGetMaxRecSize is set for dlpFindDBByOpenHandleReqArgID only:
	// (otherwise, field is set to zero)
	DWord					dwMaxRecSize;	//20;		size of largest record or resource in the database (for
...
typedef struct DlpSetDBInfoReqHdrType {
												// OFFSET
	Byte					dbID;				// 0;		database id returned by dlpOpenDB or dlpCreateDB
	Byte					bReserved;		// 1;		RESERVED -- SET TO NULL
	Word					wClrDbFlags;	// 2;		flags to clear; allowed DB flags: dlpDBFlagAppInfoDirty,
												//			dlpDBFlagBackup, dlpDBFlagOKToInstallNewer,
												//			dlpDBFlagResetAfterInstall, dlpDBFlagCopyPrevention;
												//			0 = don't change
	Word					wSetDbFlags;	// 4;		flags to set; allowed DB flags: dlpDBFlagAppInfoDirty,
												//			dlpDBFlagBackup, dlpDBFlagOKToInstallNewer,
												//			dlpDBFlagResetAfterInstall, dlpDBFlagCopyPrevention;
												//			0 = don't change
	Word					wDbVersion;		// 6;		database version; dlpSetDBInfoNoVerChange = don't change
	...
	DWord					dwType;			//32;		database type id; zero = don't change
	DWord					dwCreator;		//36;		database creator id; zero = don't change
												// TOTAL: 40 bytes										
